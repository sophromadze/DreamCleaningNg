...
describe('default cleaner hourly rate', () => {
  ...
  // Post construction used to share the heavy top rate and deliberately no longer does.
  it('pays the mid rate for post construction', () => {
    for (const name of ['Post Construction Cleaning', 'post-construction cleaning']) {
      expect(getDefaultCleanerHourlyRate(NO_DEEP, name)).toBe(21);
    }
  });

  it('pays the top rate for heavy condition', () => {
    expect(getDefaultCleanerHourlyRate(NO_DEEP, 'Heavy Condition Cleaning')).toBe(25);
    expect(getDefaultCleanerHourlyRate(NO_DEEP, 'Heavy Conditional Cleaning')).toBe(25);
  });
